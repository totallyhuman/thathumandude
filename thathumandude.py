...
# Sets nick to my inbox nick which all notifies are directed to
...
# Prints out the errors if the sender is me, the creator
	...
	if meta['sender'] == nick_mono or meta['sender'] == ':3':
		try:
			for e in range(0, len(errors) + 1):
				...
				meta['self'].send_chat('[%s, %s ago] %s' % (sender, time_since, message), meta['msgid'])
		except IndexError:
			pass
		del errors[:]
	else:
...
# Easter egg that has a 1/20 chance od sending "Stop hitting yourself!"
def facepalm(match, meta):
	if randint(1, 20) == 1:
		return 'Stop hitting yourself!'

nick_sans = u'totally\U0001D5C1\U0001D5CE\U0001D5C6\U0001D5BA\U0001D5C7' # My notify nick
nick_mono = u'totally\U0001D691\U0001D69E\U0001D696\U0001D68A\U0001D697' # My nick
